...
sed_command='s!\([^/]\)github.com/ethereum/go-ethereum!\1github.com/ryt-io/subnet-evm!g'
find . \( -name '*.go' -o -name 'go.mod' -o -name 'build_test.sh' \) -exec sed -i '' -e "${sed_command}" {} \;
for dir in ${upstream_dirs}; do
    sed_command="s!\"github.com/ryt-io/subnet-evm/${dir}\"!\"github.com/ethereum/go-ethereum/${dir}\"!g"
